	...
	(
		...
		if (directions.includes(event.code)) {
			...
			const collisions = closestCorners({
				...
				droppableRects: containers.reduce<RectMap>(
					(acc, container) => {
						if (container?.rect.current) {
							acc.set(container.id, container.rect.current);
						}
			...
			if (closestId && over?.id) {
				const newNode =
					droppableContainers.get(closestId)?.node.current;
				const activeNodeRect = droppableContainers.get(active.id)?.rect
					.current;
					...
					if (newItem && activeItem) {
						...
						const offset =
							newRect.top > activeNodeRect.top
								? Math.abs(
										activeNodeRect.height - newRect.height
								  )
								: 0;
