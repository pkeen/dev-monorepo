...
import { utils } from "./lib/auth";
import { secret } from "./lib/auth/utils/token/jwt/key";

// for testing wo middle ware
export function middleware(request: NextRequest) {
    return NextResponse.next(); // for debugging
	// console.log("Middleware running on:", request.nextUrl.pathname);
	// const accessToken = request.cookies.get("pk-auth-access")?.value;
	// if (!accessToken) {
